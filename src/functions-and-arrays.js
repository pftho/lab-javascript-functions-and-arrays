...
const words3 = ["mango", "bananas", "bananas"];
...
function findLongestWord(arr) {
  ...
  arr.forEach((element) => {
    if (element.length > longuestElement.length) {
      longuestElement = element;
    }
  });
  return longuestElement;
}
...
function sum(arr) {
  let sum = 0;
  arr.forEach((element) => {
    //if element is object or array -> error
    if (
